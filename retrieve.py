...
def load_video_frames(video_path, fps=None, max_frames=8, target_height=720):
    ...
    if fps is not None:
        ...
        if total_frames > 0:
            indices = set(np.linspace(0, total_frames - 1, min(max_frames, total_frames), dtype=int))
        else:
            indices = None
...
def main():
    parser = argparse.ArgumentParser(description="Retrieve song lyrics based on a video or image query.")
    parser.add_argument("query_path", type=str, help="Path to the video or image file (e.g., query.mp4)")
    ...
    parser.add_argument("--mood", type=str, default=None, help="Thematic preference for mood (e.g. love, fun, hilarious, adventure, chill/relax)")
    parser.add_argument("--fps", type=float, default=None, help="Frames per second to sample from the video")
    ...
    is_video = args.query_path.lower().endswith(('.mp4', '.avi', '.mov', '.mkv', '.webm'))
